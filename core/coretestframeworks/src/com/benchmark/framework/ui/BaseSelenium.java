...
public class BaseSelenium {
	...
	/**
	 * Variable to hold Action instance, which will be used to perform actions like
	 * mouse events, key up, key down, move, drag etc..
	 */
	 ...
	 * @return {@code true} if the page loaded successfully, {@code false} otherwise
	 */
	 ...
	 * 
	 * @param findByType         Selector strategy for the screen blocker
	 * @param valueToFindElement Locator string referencing the screen blocker
	 * @param counter            Maximum delay (in seconds) to wait for the blocker
	 *                           to clear
	...
	/*
	 * Wait till the element is clickable Author=Pratikshya
	 */
	public static void waitTillElementToBeClickableAndClick(String findByType, String valueToFindElement) {
		int count = 0;
		boolean flag = false;
		while (!flag) {
			try {
	 ...
	 * 
	 * @param findByType         -> FindByType
	 * @param valueToFindElement -> FindByValue
	 * @param counter            -> Counter.
	 */
	...
	/*
	 * Provide an optional timeout value for isElementAvailable. The timeout value
	 * is measured in seconds.
	 * 
	 * @param findByType {@link FindByTypeConstants} element specifying the locator
	 * strategy
	 * 
	...
	/**
	 * Test to determine if the specified element is visible in the browser without
	 * any delays.
	 * 
	 * @param findByType         Locator strategy
	 * @param valueToFindElement Locator string
	...
	/**
	 * Test to determine if the specified element is visible, with an optional retry
	 * timer. If the element is not immediately visible, this method waits one
	 * second and retries (to a limit of `timeout` seconds)
	 * 
	 * @param findByType         Locator strategy
	 * @param valueToFindElement Locator string
	 * @param timeout            The maximum number of seconds to delay while
	 *                           waiting
	...
	private static void setup() {
		...
		try {
			...
			case DriverTypeConstants.CHROME_HEAD_LESS:
				chromeExePath = null;
				if (System.getProperty("os.name").startsWith("Windows")) {
					chromeExePath = appProperties.getProperty(CommonConstants.CHROME_DRIVER_EXE);
					cap = DesiredCapabilities.chrome();
			...
			default:
				...
				if (multipleclient)
					...
					m_webDriver = new FirefoxDriver((Capabilities) createFirefoxProfile());
			}
